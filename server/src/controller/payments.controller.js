...
const createPayment = async (req, res, next) => {
  ...
  try {
    ...
    const [vendorResult] = await Query(vendorQuery, [vendor_id], [Master.vendors.prefix_]);
    ...
    try {
      ...
      const updatePromises = updates.map(async (update) => {
        ...
        if (currentState === 'PREPARED') {
          ...
          updateQuery = sql.update(Accounting.payments.tablename)

            .set([Accounting.payments.selectOptionColumns.state, Accounting.payments.selectOptionColumns.checked_by])
          ...
          updateQuery = sql.update(Accounting.payments.tablename)
            ...
            .set([Accounting.payments.selectOptionColumns.state, Accounting.payments.selectOptionColumns.approved_by])
...
const updatePayment = async (req, res, next) => {
  ...
  try {
    ...
    if (vendor_id && isNaN(Number(vendor_id))) {
      ...
      if (vendorResult && vendorResult.length > 0) {
        ...
        if (vendorResult && vendorResult.id) {
          ...
          console.log('Found vendor in result object:', vendor_id, '→', actualVendorId);
    ...
    if (!actualVendorId || !document_reference || !mode_of_payment || !payment_date || !total_amount_due) {
    ...
    try {
      ...
      const currentPaymentQuery = sql.select([

        { col: Accounting.payments.selectOptionColumns.vendor_id, as: 'vendor_id' },

        { col: Accounting.payments.selectOptionColumns.document_reference, as: 'document_reference' },
      ...
      const [currentPaymentData] = await connection.execute(currentPaymentQuery, [paymentId]);
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        currentJournalData = await Query(currentJournalQuery, ['payments', paymentId], [Accounting.journal_entries.prefix_]);
      ...
      currentAttachmentsData = await connection.execute(currentAttachmentsQuery, [paymentId]);



      const updateMainQuery = sql.update(Accounting.payments.tablename)
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        const existingEntries = await Query(existingEntriesQuery, ['payments', paymentId], [Accounting.journal_entries.prefix_]);
        ...
        if (entriesToDelete.length > 0) {
          ...
          for (const entryId of entriesToDelete) {
            ...
            await connection.execute(deleteEntriesQuery, [entryId, 'payments', paymentId]);
      ...
      if (attachments && attachments.length > 0) {
        ...
        const existingAttachmentsQuery = sql.select([
          ...
          Accounting.payment_attachments.selectOptionColumns.id
        ...
        const existingAttachments = await Query(existingAttachmentsQuery, [paymentId], [Accounting.payment_attachments.prefix_]);
        ...
        const existingAttachmentsInPayload = attachments.filter(attachment =>
        ...
        const payloadAttachmentIds = existingAttachmentsInPayload.map(attachment => attachment.id);
        ...
        for (const attachmentId of attachmentsToDelete) {
          ...
          const deleteQuery = sql.delete(Accounting.payment_attachments.tablename)
        ...
        for (const attachment of attachments) {
          ...
          const isExistingAttachment = attachment.id && 
          ...
          if (isExistingAttachment) {
            ...
            if (attachment.file && typeof attachment.file === 'string' && attachment.file.startsWith('data:')) {

              updateFields.push(Accounting.payment_attachments.selectOptionColumns.file);
            ...
            const updateAttachmentQuery = sql.update(Accounting.payment_attachments.tablename)
            ...
            const insertAttachmentQuery = sql.insert(Accounting.payment_attachments.tablename, {
      ...
      console.log('DEBUG: Request body data:', { vendor_id, document_reference, mode_of_payment, bank_name, check_number, payment_date, remarks });
      ...
      if (currentPaymentData.length > 0) {
        ...
        if (currentPaymentDate !== newPaymentDate) {
          ...
          auditChanges.push(`Payment Date: ${currentPaymentDate || 'NULL'} → ${newPaymentDate || 'NULL'}`);
      ...
      const hasPaymentItemsChanges = payment_items && 

                                payment_items.length > 0 && 

                                payment_items.some(item => 
          ...
          if (currentItem) {
            ...
            if (item.amount !== undefined && item.amount !== null) {
              ...
              if (currentAmount !== newAmount) {
                ...
                auditChanges.push(`Payment Item ${currentItem.id} Amount: ${currentAmount} → ${newAmount}`);
            ...
            if (item.purchase_id && !item.id) {
              ...
              auditChanges.push(`Payment Item: Purchase ID ${item.purchase_id} (NEW)`);
      ...
      const hasJournalChanges = journal_entries && 

                              journal_entries.length > 0 && 

                              journal_entries.some(entry => 
                                ...
                                (entry.responsibility_center !== undefined && entry.responsibility_center !== null && entry.responsibility_center !== '') // Existing entries with responsibility center changes
      ...
      if (hasJournalChanges) {
        ...
        for (let i = 0; i < journal_entries.length; i++) {
          ...
          if (entry.id) {
            ...
            currentEntry = currentJournalData?.find((existingEntry, index) => 
            ...
            if (!currentEntry && entry.account_id) {
              ...
              currentEntry = currentJournalData?.find(existingEntry => 
          ...
          if (currentEntry) {
            ...
            if (!entry.id) {
              ...
              auditChanges.push(`Journal Entry: ${normalizeValue(entry.responsibility_center) || 'Unassigned'} (NEW)`);
      ...
      if (attachments && attachments.length > 0) {
        ...
        console.log('DEBUG: Valid payload attachments:', existingAttachmentsInPayload);
        ...
        for (const attachment of attachments) {
          ...
          if (isExistingAttachment) {
            ...
            if (currentAttachment) {
              ...
              auditChanges.push(`Attachment: ${normalizeValue(attachment.name) || normalizeValue(attachment.fileName) || 'Unknown'} (NEW)`);
