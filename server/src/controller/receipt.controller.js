...
const getPrintReceipts = async (req, res, next) => {
  ...
  try {
    ...
    const receipts_items_query = sql.select([
      ...
      { col: Accounting.receipt_items.selectOptionColumns.receipts_id, as: 'receipts_id' },
    ...
    const receipts_attachments_query = sql.select([
      ...
      { col: Accounting.receipt_attachments.selectOptionColumns.receipt_id, as: 'receipt_id' },
...
const updateReceipt = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      const currentReceiptQuery = sql.select([
        ...
        { col: Accounting.receipts.selectOptionColumns.mode_of_payment, as: 'mode_of_payment' },

        { col: Accounting.receipts.selectOptionColumns.bank_name, as: 'bank_name' },

        { col: Accounting.receipts.selectOptionColumns.check_number, as: 'check_number' },

        { col: Accounting.receipts.selectOptionColumns.total_amount_due, as: 'total_amount_due' }
      ...
      const [currentReceiptData] = await connection.execute(currentReceiptQuery, [receiptId]);
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        currentJournalData = await connection.execute(currentJournalQuery, ['receipts', receiptId]);
      ...
      console.log('DEBUG: Request body data:', { document_reference, remarks, customer_id, payment_date, mode_of_payment, bank_name, check_number, total_amount_due });
      ...
      if (currentReceiptData.length > 0) {
        ...
        if (currentModePayment !== newModePayment) {
          auditChanges.push(`Mode of Payment: ${currentModePayment || 'NULL'} → ${newModePayment || 'NULL'}`);
        }
        ...
        for (const item of receipt_items) {
          ...
          if (item.id) {
            ...
            if (currentItem) {
              ...
              console.log('DEBUG: Item qty comparison:', { itemId: item.id, current: currentQty, new: newQty, changed: currentQty !== newQty });
              ...
              console.log('DEBUG: Item price comparison:', { itemId: item.id, current: currentPrice, new: newPrice, changed: currentPrice !== newPrice });
              if (currentPrice !== newPrice) {
                auditChanges.push(`Item ${item.id} Price: ${currentPrice} → ${newPrice}`);
              }
              
              const currentDesc = normalizeValue(currentItem.description);
              const newDesc = normalizeValue(item.description);
              console.log('DEBUG: Item desc comparison:', { itemId: item.id, current: currentDesc, new: newDesc, changed: currentDesc !== newDesc });
              if (currentDesc !== newDesc) {
                auditChanges.push(`Item ${item.id} Desc: ${currentDesc || 'NULL'} → ${newDesc || 'NULL'}`);
              }
              ...
              console.log('DEBUG: Item resp center comparison:', { itemId: item.id, current: currentRespCenter, new: newRespCenter, changed: currentRespCenter !== newRespCenter });
              if (currentRespCenter !== newRespCenter) {
                auditChanges.push(`Item ${item.id} Resp Center: ${currentRespCenter || 'NULL'} → ${newRespCenter || 'NULL'}`);
              }
            }
          } else {
      ...
      if (journal_entries && journal_entries.length > 0) {
        for (const entry of journal_entries) {
          ...
          if (entry.id) {
            const currentEntry = currentJournalData[0]?.find(j => j.id === entry.id);
            ...
            if (currentEntry) {
              ...
              console.log('DEBUG: Journal resp center comparison:', { entryId: entry.id, current: currentRespCenter, new: newRespCenter, changed: currentRespCenter !== newRespCenter });
              if (currentRespCenter !== newRespCenter) {
                auditChanges.push(`Journal ${entry.id} Resp Center: ${currentRespCenter || 'NULL'} → ${newRespCenter || 'NULL'}`);
              }
              
              const currentAmount = normalizeNumber(currentEntry.amount);
              const newAmount = normalizeNumber(amount);
              if (currentAmount !== newAmount) {
                auditChanges.push(`Journal ${entry.id} Amount: ${currentAmount} → ${newAmount}`);
              }
              
              const currentType = normalizeValue(currentEntry.type);
              if (currentType.toLowerCase() !== type.toLowerCase()) {
                auditChanges.push(`Journal ${entry.id} Type: ${currentType} → ${type}`);
              }
            }
          } else {
      ...
      const existingAttachmentsQuery = sql.select([
        Accounting.receipt_attachments.selectOptionColumns.id
      ])
        .from(Accounting.receipt_attachments.tablename)
        .where(Accounting.receipt_attachments.selectOptionColumns.receipt_id)
        .build();

      const existingAttachments = await Query(existingAttachmentsQuery, [receiptId], [Accounting.receipt_attachments.prefix_]);
      ...
      const currentAttachmentsData = await connection.execute(currentAttachmentsQuery, [receiptId]);

      if (attachments && attachments.length > 0) {
        const payloadAttachmentIds = attachments.filter(attachment => attachment.id).map(attachment => attachment.id);
        ...
        for (const attachment of attachments) {
          if (attachment.id) {
            ...
            auditChanges.push(`Added new attachment: ${normalizeValue(attachment.fileName) || 'Unknown'}`);
          }
        }
      } else {
    ...
    try {
      ...
      const updatePromises = updates.map(async (update) => {
        ...
        if (currentState === 'PREPARED') {
          ...
          updateQuery = sql.update(Accounting.receipts.tablename)
            .set([Accounting.receipts.selectOptionColumns.state, Accounting.receipts.selectOptionColumns.checked_by])
            .where(Accounting.receipts.selectOptionColumns.id)
            .build();
          ...
          updateQuery = sql.update(Accounting.receipts.tablename)
            .set([Accounting.receipts.selectOptionColumns.state, Accounting.receipts.selectOptionColumns.approved_by])
            .where(Accounting.receipts.selectOptionColumns.id)
            .build();
        ...
        const currentDataQuery = sql.select([
          { col: Accounting.receipts.selectOptionColumns.document_reference, as: 'doc_ref' },
          { col: Accounting.receipts.selectOptionColumns.remarks, as: 'remarks' }
        ])
          .from(Accounting.receipts.tablename)
        ...
        const currentJournal = await connection.execute(currentJournalQuery, ['receipts', id]);
        ...
        if (changes) {
          ...
          if (changes.document_reference && currentData.length > 0) {
            ...
            if (currentDocRef !== newDocRef) {
              ...
              const docRefUpdateQuery = sql.update(Accounting.receipts.tablename)
                .set([Accounting.receipts.selectOptionColumns.document_reference])
                .where(Accounting.receipts.selectOptionColumns.id)
                .build();

              await connection.execute(docRefUpdateQuery, [changes.document_reference, id]);
            }
          }
          ...
          if (changes.remarks && currentData.length > 0) {
            ...
            if (currentRemarks !== newRemarks) {
              ...
              const remarksUpdateQuery = sql.update(Accounting.receipts.tablename)
      ...
      res.status(200).json({
        ...
        data: {
          ...
          updates: results.map(result => ({ id: result.id, nextState: result.nextState }))
        },
        timestamp: new Date().toISOString()
      });
