    ...
    try {
      ...
      if (postedSafeEdit) {
        ...
        return res.status(200).json({
          success: true,
          message: `Updated SAFE fields only (${postedSafeEdit.changedSafe.join(', ') || 'attachments'}) on ${postedSafeEdit.doc.state} Cash Disbursement ${disbursementId}. Journal entries were left unchanged.`,
          data: {
            id: disbursementId,
      ...
      await assertEditableDocument(connection, 'cash_disbursements', disbursementId)
...
const cancelDisbursementState = async (req, res, next) => {
  try {
    const { updates } = req.body
    ...
    try {
      ...
      if (validUpdates.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            'No disbursements eligible for cancellation. Only disbursements not already CANCELLED, REJECTED or APPROVED/POSTED can be cancelled.',
      ...
      res.status(200).json({
        success: true,
        message: `${validUpdates.length} disbursement(s) cancelled successfully`,
