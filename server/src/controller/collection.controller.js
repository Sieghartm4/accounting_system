  ...
  try {
    ...
    const query = sql
      .select([
        ...
        {
          col: `${Accounting.collections.tablename}.c_mode_of_payment`,
          as: 'mode_of_payment',
        },
        ...
        {
          col: `${Accounting.collections.tablename}.c_check_number`,
          as: 'check_number',
        },

        {
          col: `${Accounting.collections.tablename}.c_collection_date`,
          as: 'collection_date',
        },
      ...
      .innerJoin(
        ...
        `${Accounting.collections.tablename}.c_customer_id`,
        `${Master.customers.tablename}.c_id`,
      )
    ...
    res.status(200).json({
      ...
      hasMore: shouldPaginate ? collections.length === limitNum : false,
...
const getAllCollections = async (req, res, next) => {
  ...
  try {
    const collection_query = sql
      .select([
        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.id}`,
          as: 'id',
        },

        {
          col: `${Master.customers.tablename}.${Master.customers.selectOptionColumns.name}`,
          as: 'customer',
        },
        ...
        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.mode_of_payment}`,
          as: 'mode_of_payment',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.bank_name}`,
          as: 'bank_name',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.check_number}`,
          as: 'check_number',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.collection_date}`,
          as: 'collection_date',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.remarks}`,
          as: 'remarks',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.state}`,
          as: 'state',
        },
      ])
      ...
      .innerJoin(
        Master.customers.tablename,
        `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.customer_id}`,
    ...
    const collection_attachments_query = sql
      .select([
        { col: Accounting.collection_attachments.selectOptionColumns.id, as: 'id' },

        {
          col: Accounting.collection_attachments.selectOptionColumns.name,
          as: 'name',
        },

        {
          col: Accounting.collection_attachments.selectOptionColumns.file,
          as: 'file',
        },

        {
          col: Accounting.collection_attachments.selectOptionColumns.remarks,
          as: 'remarks',
        },

        {
          col: Accounting.collection_attachments.selectOptionColumns.uploaded_by,
          as: 'uploaded_by',
        },

        {
          col: Accounting.collection_attachments.selectOptionColumns.uploaded_date,
          as: 'uploaded_date',
        },
      ])
  ...
  try {
    ...
    if (!customer_id || !mode_of_payment || !collection_date || !created_by) {
      return res.status(400).json({
        ...
        message:
          'Missing required fields: customer_id, document_reference, mode_of_payment, collection_date, created_by',
      })
    }
    ...
    try {
      ...
      const transactionDateKey = getPeriodKeyForDate(
        String(collection_date || new Date().toISOString().slice(0, 10)).slice(0, 10),
      )
      assertPeriodIsOpen(await findPeriodForDateKey(connection, transactionDateKey), 'create a collection')
        ...
        for (const item of collection_items) {
          ...
          if (!item.sales_id) {
            ...
            return res.status(400).json({
              success: false,
              message:
                'Collection items must have a sales_id. Each collection item must be linked to a sales record.',
            })
          }
      ...
      if (attachments && attachments.length > 0) {
        for (const attachment of attachments) {
          const attachmentQuery = sql
            .insert(Accounting.collection_attachments.tablename, {
      ...
      setImmediate(async () => {
        try {
          const selectNewCollectionQuery = sql
            .select([
              {
                col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.id}`,
                as: 'id',
          ...
          const createdCollectionRows = await Query(
            selectNewCollectionQuery,
            [collectionId],
            [Accounting.collections.prefix_, Master.customers.prefix_],
          )
          const createdCollection = Array.isArray(createdCollectionRows)
            ? createdCollectionRows[0]
            : createdCollectionRows

          if (createdCollection) {
            broadcastUpdates({ collection: createdCollection }, 'collection_created')
          }
        } catch (err) {
          console.error('Error broadcasting collection creation:', err)
        }
      })
    } catch (error) {
      if (connection) {
      ...
      const updatePromises = updates.map(async (update) => {
        ...
        if (currentState === 'PREPARED') {
          ...
          updateQuery = sql
            ...
            .set([
              Accounting.collections.selectOptionColumns.state,
              Accounting.collections.selectOptionColumns.checked_by,
            ])
          ...
          updateQuery = sql
            ...
            .set([
              Accounting.collections.selectOptionColumns.state,
              Accounting.collections.selectOptionColumns.approved_by,
            ])
  ...
  try {
    ...
    const collections_query = sql
      .select([
        ...
        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.customer_id}`,
          as: 'customer_id',
        },
        ...
        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.created_by}`,
          as: 'created_by',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.checked_by}`,
          as: 'checked_by',
        },

        {
          col: `${Accounting.collections.tablename}.${Accounting.collections.selectOptionColumns.approved_by}`,
          as: 'approved_by',
        },
      ])
    ...
    const collection_items_query = sql
      ...
      .whereIn(
        Accounting.collection_items.selectOptionColumns.collection_id,
        collectionIds,
      )
    ...
    const collection_attachments_query = sql
      .select([
        ...
        {
          col: Accounting.collection_attachments.selectOptionColumns.collection_id,
          as: 'collection_id',
        },
      ...
      .whereIn(
        Accounting.collection_attachments.selectOptionColumns.collection_id,
        collectionIds,
      )
    ...
    const groupedData = collections.map((collection) => {
      ...
      return {
        ...
        attachments: collection_attachments.filter(
          (att) => att.collection_id === collection.id,
        ),
...
const updateCollection = async (req, res, next) => {
  ...
  try {
    ...
    if (customer_id && isNaN(Number(customer_id))) {
      ...
      if (customerResult && customerResult.length > 0) {
        ...
        if (customerResult && customerResult.id) {
          ...
          console.log(
            'Found customer in result object:',
            customer_id,
            '→',
            actualCustomerId,
          )
    ...
    try {
      ...
      const currentCollectionQuery = sql
        .select([
          {
            col: Accounting.collections.selectOptionColumns.customer_id,
            as: 'customer_id',
          },

          {
            col: Accounting.collections.selectOptionColumns.document_reference,
            as: 'document_reference',
          },
          ...
          {
            col: Accounting.collections.selectOptionColumns.check_number,
            as: 'check_number',
          },

          {
            col: Accounting.collections.selectOptionColumns.collection_date,
            as: 'collection_date',
          },
      ...
      const currentAttachmentsQuery = sql
        .select([
          {
            col: Accounting.collection_attachments.selectOptionColumns.id,
            as: 'id',
          },
      ...
      if (attachments && attachments.length > 0) {
        const existingAttachmentsQuery = sql
          .select([Accounting.collection_attachments.selectOptionColumns.id])
        ...
        for (const attachment of attachments) {
          ...
          if (attachment.id && isExistingAttachment) {
            const updateAttachmentQuery = sql
              .update(Accounting.collection_attachments.tablename)
      ...
      if (currentCollectionData.length > 0) {
        ...
        if (current.customer_id !== actualCustomerId) {
          auditChanges.push(
            `Customer ID: ${current.customer_id} → ${actualCustomerId}`,
          )
        }
          ...
          if (currentItem) {
            ...
            if (item.amount !== undefined && item.amount !== null) {
              ...
              if (currentAmount !== newAmount) {
                auditChanges.push(
                  `Collection Item ${currentItem.id} Amount: ${currentAmount} → ${newAmount}`,
                )
              }
            }
          } else {
            ...
            if (item.sales_id) {
              auditChanges.push(
                `Added new collection item: Sales ID ${item.sales_id}`,
              )
            }
          }
        }
