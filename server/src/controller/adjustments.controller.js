    ...
    try {
      ...
      const updatePromises = updates.map(async (update) => {
        ...
        if (currentState === 'PREPARED') {
          nextState = 'CHECKED';
          updateQuery = sql.update(Accounting.adjustments.tablename)
            .set([Accounting.adjustments.selectOptionColumns.status, Accounting.adjustments.selectOptionColumns.checked_by])
            .where(Accounting.adjustments.selectOptionColumns.id)
            .build();
          ...
          nextState = 'APPROVED';
          updateQuery = sql.update(Accounting.adjustments.tablename)
            .set([Accounting.adjustments.selectOptionColumns.status, Accounting.adjustments.selectOptionColumns.approved_by])
            .where(Accounting.adjustments.selectOptionColumns.id)
            .build();
          updateValues = [nextState, req.context.username, id];
        } else {
...
const updateAdjustmentData = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const currentAdjustmentQuery = sql.select([
        { col: Accounting.adjustments.selectOptionColumns.document_reference, as: 'document_reference' },
        { col: Accounting.adjustments.selectOptionColumns.posting_date, as: 'posting_date' },
        { col: Accounting.adjustments.selectOptionColumns.remarks, as: 'remarks' },
        { col: Accounting.adjustments.selectOptionColumns.total_amount, as: 'total_amount' }
      ...
      const [currentAdjustmentData] = await connection.execute(currentAdjustmentQuery, [adjustmentId]);
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        currentJournalData = await connection.execute(currentJournalQuery, ['adjustments', adjustmentId]);
      }
      ...
      currentAttachmentsData = await connection.execute(currentAttachmentsQuery, [adjustmentId]);
      ...
      console.log('DEBUG: Request body data:', { document_reference, posting_date, remarks, total_amount });
      ...
      if (currentAdjustmentData.length > 0) {
        ...
        if (currentPostingDate !== newPostingDate) {
          auditChanges.push(`Posting Date: ${currentPostingDate || 'NULL'} → ${newPostingDate || 'NULL'}`);
        }
        ...
        if (normalizeNumber(current.total_amount) !== normalizeNumber(total_amount)) {
          auditChanges.push(`Total Amount: ${current.total_amount} → ${total_amount}`);
        }
      }
      ...
      const existingAttachmentsQuery = sql.select([
        Accounting.adjustment_attachments.selectOptionColumns.id
      ])
        .from(Accounting.adjustment_attachments.tablename)
        .where(Accounting.adjustment_attachments.selectOptionColumns.adjustment_id)
        .build();

      const existingAttachments = await Query(existingAttachmentsQuery, [adjustmentId], [Accounting.adjustment_attachments.prefix_]);
      ...
      if (adjustment_attachments && adjustment_attachments.length > 0) {
        ...
        const validAttachments = adjustment_attachments.filter(attachment =>
          attachment.id &&
          attachment.id !== null &&
