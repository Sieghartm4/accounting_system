...
const updateSale = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      const currentSalesQuery = sql.select([
        ...
        { col: Accounting.sales.selectOptionColumns.total_amount_due, as: 'total_amount_due' }
      ...
      const [currentSalesData] = await connection.execute(currentSalesQuery, [salesId]);
      ...
      if (sales_items && sales_items.length > 0) {
        ...
        const currentItemsQuery = sql.select([
          ...
          { col: Accounting.sales_items.selectOptionColumns.charts_of_accounts, as: 'account_id' },
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        currentJournalData = await connection.execute(currentJournalQuery, ['sales', salesId]);
      ...
      currentAttachmentsData = await connection.execute(currentAttachmentsQuery, [salesId]);
      ...
      if (attachments && attachments.length > 0) {
        ...
        if (attachmentsToDelete.length > 0) {
          ...
          const attachmentsToDeleteQuery = sql.select([
            Accounting.sales_attachments.selectOptionColumns.id,
          ...
          const attachmentsToDeleteData = await Query(attachmentsToDeleteQuery, [...attachmentsToDelete, salesId], [Accounting.sales_attachments.prefix_]);
        ...
        const existingAttachmentsForDeleteQuery = sql.select([
          Accounting.sales_attachments.selectOptionColumns.name
        ])
          .from(Accounting.sales_attachments.tablename)
          .where(Accounting.sales_attachments.selectOptionColumns.sales_id)
        ...
        const existingAttachmentsForDelete = await Query(existingAttachmentsForDeleteQuery, [salesId], [Accounting.sales_attachments.prefix_]);
      ...
      console.log('DEBUG: Request body data:', { customer_id, document_reference, terms, date_delivered, date_due, remarks, total_amount_due });
