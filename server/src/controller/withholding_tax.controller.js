...
const updateWithholdingTax = async (req, res, next) => {
  try {
    ...
    const existingQuery = sql.select([Master.withholding_tax.selectOptionColumns.code, Master.withholding_tax.selectOptionColumns.name, Master.withholding_tax.selectOptionColumns.rate, Master.withholding_tax.selectOptionColumns.tax_account, Master.withholding_tax.selectOptionColumns.description, Master.withholding_tax.selectOptionColumns.status])
      .from(Master.withholding_tax.tablename)
      .where(Master.withholding_tax.selectOptionColumns.id)
      .build();
    const existingTaxes = await Query(existingQuery, [id], Master.withholding_tax.prefix_);
    ...
    try {
      ...
      if (old.tax_account !== tax_account) changes.push(`tax_account='${tax_account}'`);
