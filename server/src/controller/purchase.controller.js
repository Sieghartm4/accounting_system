...
const regeneratePaymentsJournalEntries = async (connection, paymentIds = []) => {
  ...
  for (const paymentId of paymentIds) {
    ...
    const paymentAccountId = await resolvePaymentAccountId(
...
const getPurchase = async (req, res, next) => {
  ...
  try {
    ...
    const { status, forPayments } = req.query || {}
    ...
    // Only apply payments filter when requested by payments page

    if (forPayments) {

      // Filter out purchases where payments sum equals or exceeds amount due (regardless of payment state)
      ...
      whereClause += ` WHERE (${Accounting.purchase.selectOptionColumns.total_amount_due} - COALESCE((${paymentsSubquery}), 0)) > 0`

      whereClause += ` AND ${Accounting.purchase.selectOptionColumns.state} = 'APPROVED'`



      console.log('Payments filter whereClause:', whereClause)
    ...
    res.status(200).json({
      ...
      hasMore: shouldPaginate ? purchases.length === limitNum : false,
...
const getAllPurchase = async (req, res, next) => {
  ...
  try {
    ...
    const purchase_items_query = sql
      ...
      .select([
...

        {
          ...
          col: Accounting.purchase_items.selectOptionColumns.withholding_tax,
    ...
    let purchase_items = await Query(
    ...
    const purchase_journal_query = sql
    ...
    let purchase_journal = await Query(

      purchase_journal_query,
    ...
    const purchase_attachments_query = sql
      ...
      .select([
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.file,
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.name,
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.remarks,
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.uploaded_by,
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.uploaded_date,
    ...
    let purchase_attachments = await Query(

      purchase_attachments_query,
  ...
  try {
    ...
    try {
      ...
      const mainValues = [
        ...
        0, // paid_amount - default to 0 for new purchases
      ...
      if (attachments && attachments.length > 0) {
        ...
        for (const attachment of attachments) {
          ...
          const attachmentQuery = sql
            ...
            .insert(Accounting.purchase_attachments.tablename, {
      ...
      setImmediate(async () => {
        ...
        try {
          ...
          const selectNewPurchaseQuery = sql
            ...
            .select([
...

              {
                ...
                col: Accounting.purchase.selectOptionColumns.date_due,
...

              {
                ...
                col: Accounting.purchase.selectOptionColumns.remarks,
          ...
          const createdPurchaseRows = await Query(

            selectNewPurchaseQuery,
          ...
          const createdPurchase = Array.isArray(createdPurchaseRows)

            ? createdPurchaseRows[0]
          ...
          if (createdPurchase) {
            ...
            broadcastUpdates({ purchase: createdPurchase }, 'purchase_created')

          }

        } catch (err) {

          console.error('Error broadcasting purchase creation:', err)
...
const updatePurchase = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      const currentPurchaseQuery = sql

        .select([

          {

            col: Accounting.purchase.selectOptionColumns.vendor_id,
      ...
      const [currentPurchaseData] = await connection.execute(currentPurchaseQuery, [
        ...
        const currentItemsQuery = sql
          ...
          .select([
...

            {
              ...
              col: Accounting.purchase_items.selectOptionColumns
      ...
      if (purchase_items && purchase_items.length > 0) {
        ...
        const existingItemsQuery = sql
          ...
          .select([Accounting.purchase_items.selectOptionColumns.id])
        ...
        if (purchase_items && purchase_items.length > 0) {
          ...
          const payloadItemIds = purchase_items
          ...
          itemsToDelete = existingItemIds.filter(
        ...
        if (itemsToDelete.length > 0) {
          ...
          const affectedPaymentsQuery = sql
          ...
          const affectedPaymentIds = [
            ...
            ...new Set(affectedPayments.map((r) => r.payment_id)),
          ...
          if (affectedPaymentIds.length > 0) {
            ...
            await connection.execute(deletePaymentJournalSql, [
          ...
          const deletePaymentItemsQuery = sql
        ...
        for (const item of purchase_items) {
          ...
          if (item.id) {
            ...
            const updateItemQuery = sql
              ...
              .update(Accounting.purchase_items.tablename)
        ...
        req.body.hasOwnProperty('purchase_items') &&
        ...
        const purchaseItemIdsQuery = sql
        ...
        const purchaseItemIds = await Query(

          purchaseItemIdsQuery,
      ...
      if (journal_entries && journal_entries.length > 0) {
        ...
        if (journal_entries && journal_entries.length > 0) {
          ...
          entriesToDelete = existingEntryIds.filter(
            ...
            if (entry.id) {
              ...
              const updateEntryQuery = sql
                ...
                .set([
                  ...
                  Accounting.journal_entries.selectOptionColumns
        ...
        req.body.hasOwnProperty('journal_entries') &&
      ...
      if (attachments && attachments.length > 0) {
        ...
        const existingAttachmentsQuery = sql
          ...
          .select([Accounting.purchase_attachments.selectOptionColumns.id])
        ...
        if (attachmentsToDelete.length > 0) {
          ...
          const deleteAttachmentsQuery = sql
            ...
            .andWhere(
              ...
              Accounting.purchase_attachments.selectOptionColumns.purchase_id,
        ...
        for (const attachment of attachments) {
          ...
          if (isValidDbId) {
            ...
            const updateAttachmentQuery = sql
              ...
              .update(Accounting.purchase_attachments.tablename)
        ...
        req.body.hasOwnProperty('attachments') &&
...
const getPrintPurchases = async (req, res, next) => {
  ...
  try {
    ...
    const purchases_query = sql

      .select([

        {

          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.id}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.vendor_id}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.document_reference}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.terms}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.date_delivered}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.date_due}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.remarks}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.total_amount_due}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.status}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.state}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.created_by}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.checked_by}`,
...

        {
          ...
          col: `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.approved_by}`,
      ...
      .innerJoin(
        ...
        `${Accounting.purchase.tablename}.${Accounting.purchase.selectOptionColumns.vendor_id}`,
    ...
    const purchase_attachments_query = sql
      ...
      .select([
...

        {
          ...
          col: Accounting.purchase_attachments.selectOptionColumns.purchase_id,
    ...
    const groupedData = purchases.map((purchase) => {
      ...
      return {
        ...
        attachments: purchase_attachments.filter(

          (att) => att.purchase_id === purchase.id,
