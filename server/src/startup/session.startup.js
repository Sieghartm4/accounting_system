...
const getCookieDomain = () => {
  ...
  // IP addresses and localhost cannot use a parent-domain cookie.
  if (!configuredHost || configuredHost === 'localhost' || isIpv4 || isIpv6) {
    return undefined
  }

  // Derive example.com from app.example.com. For two-label hosts, omit the
  // domain because the host-only cookie is already correct.
