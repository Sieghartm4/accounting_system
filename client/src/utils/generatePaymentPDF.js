...
export async function generatePaymentPDF(paymentData, copyType = 'internal') {
  ...
  for (let idx = 0; idx < payments.length; idx++) {
    ...
    doc.text('PAYMENT VOUCHER', margin, y + 14)

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(14)
    doc.setTextColor(...RED)
    doc.text('# ' + (payment.id ?? ''), margin, y + 26)
    ...
    doc.setFontSize(8)
    doc.setTextColor(...DGRAY)
    doc.text(formatTin(paymentTin), margin, y + 12)
    if (paymentAddress) {
      doc.text(paymentAddress, margin, y + 24)
      y += 12
    }
    ...
    // Left — payment reference + copy type
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(6.5)
    doc.setTextColor(...WHITE)
    doc.text(
      'Payment #' + (payment.id ?? idx) + '  ·  ' + copyLabel,
      margin + 8,
