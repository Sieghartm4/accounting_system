...
export async function generateReceiptPDF(receiptData, copyType = 'internal') {
  ...
  for (let idx = 0; idx < receipts.length; idx++) {
    ...
    // ── COLOUR PALETTE — red is now used ONLY as a thin-line / small-text
    //    accent, never as a large fill, to keep ink usage low. Black text on
    //    white is the workhorse; a hairline gray is used for structural rules.
    const RED = [176, 0, 32]
    const BLACK = [0, 0, 0]
    ...
    // ── LETTERHEAD BAND — new: a full-bleed red band across the very top of
    //    the page, company name in white, receipt title in white on the right.
    //    (Previously this was a plain white header with a black title.) ───────
    const BAND_H = 46
    ...
    if (company.logo) {
      try {
        // white plate behind the logo so it reads on the red band
    ...
    // Title block — right side of the band, white text, black number chip
    const titleRightX = pageW - margin

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(15)
    doc.setTextColor(...WHITE)
    doc.text('RECEIPT', titleRightX, BAND_H / 2 - 3, { align: 'right' })
    ...
    // Receipt number chip — centered beneath the header band
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    const noText = 'No. ' + (receipt.id ?? '')
    ...
    // ── DOC META (left) + CUSTOMER (right) ──────────────────────────────────────
    let collectionDate = '—'
    if (receipt.collection_date) {
      try {
        collectionDate = new Date(receipt.collection_date).toLocaleDateString(
          'en-US',
    ...
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    doc.text(receipt.customer || '—', custRightX, y + 13, { align: 'right' })
    ...
    // ── ITEMS TABLE — white header with black text + red rule under the head
    //    (was a solid red fill), keeps ink low while still reading as "red/black"
    ...
    // ── REMARKS — small red caps label, no fill (was a solid red tab) ─────────
    if (receipt.remarks) {
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(6.5)
      doc.setTextColor(...RED)
      ...
      doc.setFontSize(7.5)
      doc.setTextColor(...BLACK)
      const wrapped = doc.splitTextToSize(receipt.remarks, contentW * 0.6)
    ...
    // ── AMOUNT BREAKDOWN — now a single bordered container box with a solid
    //    header bar, a two-column layout inside, and a full-width highlighted
    //    "due" row at the bottom (previously a borderless ledger list). ───────
    ...
    // Outer border - use hairline to match table and due row
    doc.setDrawColor(...HAIRLINE)
    doc.setLineWidth(0.5)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ── AUTHORIZATION / SIGNATURE SECTION — thin hairline columns, no fills.
    //    Order: Approved → Checked → Submitted. Line sits above the name.
    // ─────────────────────────────────────────────────────────────────────────
    y += 16
    ...
    const signFields = [
      { label: 'SUBMITTED BY', value: receipt.created_by },
      { label: 'CHECKED BY', value: receipt.checked_by },
      { label: 'APPROVED BY', value: receipt.approved_by },
    ]
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ── FOOTER — full-bleed red band mirroring the top letterhead
    //    (previously just a hairline with no fill) ───────────────────────────
    // ─────────────────────────────────────────────────────────────────────────
  ...
  // After all receipts are processed, open the single multi-page PDF
  ...
  if (newWindow) {
    ...
    doc.save('receipts_' + copyType + '.pdf')
  }
}
