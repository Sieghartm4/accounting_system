// generatePurchasePDF.js
// Place this in: src/utils/generatePurchasePDF.js
// Install deps: npm install jspdf jspdf-autotable

export async function generatePurchasePDF(purchaseData, copyType = 'internal') {
  ...
  // Debug logging
  console.log('[PDF Generator] Received purchases:', purchases)

  // Create a single PDF document with multiple pages
  const doc = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'letter' })

  for (let idx = 0; idx < purchases.length; idx++) {
    // Add new page for each purchase except the first one
    if (idx > 0) {
      doc.addPage()
    }
    ...
    doc.setFontSize(14)
    doc.setTextColor(...WHITE)
    doc.text('PURCHASE ORDER', titleRightX, BAND_H / 2 - 3, { align: 'right' })
    ...
    // Purchase Order ID number chip — centered beneath the header band
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    const noText = 'No. ' + (purchase.id ?? '')
    ...
    // ── DOC META (left) + VENDOR INFO (right)
    let dateDelivered = '—'
    if (purchase.date_delivered) {
      try {
        dateDelivered = new Date(purchase.date_delivered).toLocaleDateString('en-US', {
          month: 'short',
          day: '2-digit',
          year: 'numeric',
        })
    ...
    let dateDue = '—'
    if (purchase.date_due) {
      try {
        dateDue = new Date(purchase.date_due).toLocaleDateString('en-US', {
          month: 'short',
          day: '2-digit',
          year: 'numeric',
        })
    ...
    doc.setFontSize(6.5)
    doc.setTextColor(...RED)
    doc.text('VENDOR INFORMATION', custRightX, y, { align: 'right' })

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    doc.text(vendorName, custRightX, y + 13, { align: 'right' })
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        11: { halign: 'right', cellWidth: 38 },
        12: { halign: 'right', cellWidth: 44 },
      },
    ...
    if (purchase.remarks) {
      ...
      doc.setFontSize(7.5)
      doc.setTextColor(...BLACK)
      const wrapped = doc.splitTextToSize(purchase.remarks, contentW * 0.6)
    ...
    const amtDueTotal = parseFloat(purchase.amount_due || 0)
    ...
    // ── JOURNAL TABLE
    const journal = Array.isArray(purchase.journal) ? purchase.journal : []
    ...
    doc.setTextColor(...WHITE)
    doc.text(
      copyLabel + '  ·  Purchase Order #' + (purchase.id ?? ''),
      pageW - margin,
      footerTextY,
      { align: 'right' },
    )
  ...
  // After all purchases are processed, open the single multi-page PDF
  ...
  if (newWindow) {
    ...
    doc.save('purchase_orders_' + copyType + '.pdf')
  }
}