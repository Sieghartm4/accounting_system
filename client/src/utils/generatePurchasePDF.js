...
export async function generatePurchasePDF(purchaseData, copyType = 'internal') {
  ...
  for (let idx = 0; idx < purchases.length; idx++) {
    ...
    doc.text('PURCHASE ORDER', margin, y + 14)

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(14)
    doc.setTextColor(...RED)
    doc.text('# ' + (purchase.id ?? ''), margin, y + 26)

    // Left – copy label (italic, small)
    doc.setFont('helvetica', 'italic')
    doc.setFontSize(7.5)
    doc.setTextColor(...DGRAY)
    doc.text(copyLabel, margin, y + 38)
    ...
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    doc.text(purchase.vendor || '—', margin, y)

    doc.setFont('helvetica', 'normal')
    doc.setFontSize(8)
    doc.setTextColor(...DGRAY)
    doc.text(formatTin(purchaseTin), margin, y + 12)
    if (purchaseAddress) {
    ...
    if (purchase.date_delivered) {
      try {
        dateDelivered = new Date(purchase.date_delivered).toLocaleDateString(
          'en-US',
          {
            month: 'short',
            day: '2-digit',
    ...
    if (purchase.remarks) {
      ...
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...DGRAY)
      const wrapped = doc.splitTextToSize(purchase.remarks, contentW * 0.5)
    ...
    signFields.forEach((field, i) => {
      ...
      doc.setFontSize(6)
      doc.setTextColor(...MGRAY)
      doc.text('Signature over Printed Name', sx + colW / 2, lineY + 9, {
        align: 'center',
      })
    })
