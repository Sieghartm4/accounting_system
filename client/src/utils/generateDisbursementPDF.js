...
export async function generateDisbursementPDF(
  ...
  const disbursements = Array.isArray(disbursementData)
    ? disbursementData
    : [disbursementData]
  ...
  for (let idx = 0; idx < disbursements.length; idx++) {
    // Add new page for each disbursement except the first one
    if (idx > 0) {
      doc.addPage()
    }
    ...
    doc.setFontSize(14)
    doc.setTextColor(...WHITE)
    doc.text('CASH DISBURSEMENT', titleRightX, BAND_H / 2 - 3, { align: 'right' })
    ...
    // Disbursement number chip — centered beneath the header band
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    const noText = 'No. ' + (disbursement.id ?? '')
    ...
    if (dateField) {
      try {
        paymentDate = new Date(dateField).toLocaleDateString(
          'en-US',
    ...
    doc.setFontSize(6.5)
    doc.setTextColor(...RED)
    doc.text('DISBURSED TO', vendRightX, y, { align: 'right' })

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    doc.text(vendorName, vendRightX, y + 13, { align: 'right' })
    ...
    if (disbursement.remarks) {
      ...
      doc.setFontSize(7.5)
      doc.setTextColor(...BLACK)
      const wrapped = doc.splitTextToSize(disbursement.remarks, contentW * 0.6)
    ...
    doc.setTextColor(...WHITE)
    doc.text(
      copyLabel + '  ·  Disbursement #' + (disbursement.id ?? ''),
      pageW - margin,
      footerTextY,
      { align: 'right' },
    )
  ...
  // After all disbursements are processed, open the single multi-page PDF
  ...
  if (newWindow) {
    ...
    doc.save('disbursements_' + copyType + '.pdf')
  }
}