...
export async function generateDisbursementPDF(
  ...
  for (let idx = 0; idx < disbursements.length; idx++) {
    ...
    doc.text('CASH DISBURSEMENT', margin, y + 14)

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(14)
    doc.setTextColor(...RED)
    doc.text('#' + (disbursement.id ?? ''), margin, y + 26)
    ...
    const disbursementAddress =
      ...
      ''
    doc.text(formatTin(disbursementTin), margin, y + 12)
    ...
    // ── SAVE ──────────────────────────────────────────────────────────────────
    doc.save(
      'cash_disbursement_' + (disbursement.id ?? idx) + '_' + copyType + '.pdf',
    )
  }
}
