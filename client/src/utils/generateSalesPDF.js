...
export async function generateSalesPDF(salesData, copyType = 'internal') {
  ...
  // Debug logging
  console.log('[PDF Generator] Received sales:', sales)
  sales.forEach((s, idx) => {
    console.log(`[PDF Generator] Sale ${idx}:`, {
      id: s.id,
      customer: s.customer_name || s.customer,
      date: s.date_delivered || s.created_at,
  ...
  for (let idx = 0; idx < sales.length; idx++) {
    // Add new page for each sale except the first one
    if (idx > 0) {
      doc.addPage()
    }
    ...
    doc.setFontSize(14)
    doc.setTextColor(...WHITE)
    doc.text('SALES INVOICE', titleRightX, BAND_H / 2 - 3, { align: 'right' })
    ...
    // Sales invoice number chip — centered beneath the header band
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9)
    doc.setTextColor(...BLACK)
    const noText = 'No. ' + (sale.id ?? '')
    ...
    doc.setFontSize(6.5)
    doc.setTextColor(...RED)
    doc.text('BILL TO', custRightX, y, { align: 'right' })
    ...
    doc.setFontSize(8)
    doc.setTextColor(...MGRAY)
    doc.text(formatTin(saleTin), custRightX, y + 25, { align: 'right' })

    let addressLines = []
    if (saleAddress) {
      addressLines = doc.splitTextToSize(saleAddress, maxAddressWidth)
    ...
    if (sale.remarks) {
      ...
      doc.setFontSize(7.5)
      doc.setTextColor(...BLACK)
      const wrapped = doc.splitTextToSize(sale.remarks, contentW * 0.6)
    ...
    const signFields = [
      { label: 'SUBMITTED BY', value: sale.created_by },
      { label: 'CHECKED BY', value: sale.checked_by },
      { label: 'APPROVED BY', value: sale.approved_by },
    ]
    ...
    doc.setTextColor(...WHITE)
    doc.text(
      copyLabel + '  ·  Sales Invoice #' + (sale.id ?? ''),
      pageW - margin,
      footerTextY,
      { align: 'right' },
    )
  ...
  // After all sales are processed, open the single multi-page PDF
  ...
  if (newWindow) {
    ...
    doc.save('sales_invoices_' + copyType + '.pdf')
  }
}
