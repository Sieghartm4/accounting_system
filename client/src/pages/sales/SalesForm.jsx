...
export default function SalesForm({ onBack, onSuccess, isViewMode = false, salesData = null }) {
  ...
  const addSalesItem = (isOther = false) => setSalesItems(prev => [...prev, { id: Date.now(), productId: '', productSearch: '', coa: '', coaSearch: '', description: '', qty: 1, price: 0, discount: 0, discountType: 'PERCENT', vat: 0, wht: 0, responsibilityCenter: '', isOther }]);
  ...
  const handlePostTransaction = async () => {
    try {
      ...
      if (!hasValidItems) {
        setToast({ type: 'warning', message: 'Please add at least one valid sales item' });
        return;
      }
      ...
      <div className="flex-1 flex flex-col gap-2 min-h-0">
        ...
        <fieldset className="bg-black rounded-2xl p-3 pl-6 pr-6 text-white shadow-xl">
          ...
          <div className="grid gap-3 grid-cols-5">
            ...
            <div className="col-span-1">
              <fieldset>
                <legend className="text-[11px] font-black uppercase text-gray-100">Terms <span className="text-red-600">*</span></legend>
                <div className="grid grid-cols-2 gap-1">
                  <select
                    disabled={isViewMode}
                    value={termsOption}
                    onChange={e => setTermsOption(e.target.value)}
                    className={`w-full px-2 py-1.5 rounded-lg text-[12px] font-bold outline-none transition-all ${isViewMode
                      ? 'bg-gray-100 border border-gray-300 text-black cursor-not-allowed'
                      : 'bg-white border border-gray-200 text-black focus:ring-1 focus:ring-red-500'
                    }`}
                  >
        ...
        <main className="flex-1 overflow-y-auto custom-table-scroller space-y-4 pr-1 min-h-0">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="space-y-4">
            ...
            <TableSection title="Sales Items" icon={<Wallet size={14} />}>
              ...
              <div ref={salesItemsScrollRef} className="overflow-x-auto custom-table-scroller">
                        ...
                        <td className="py-1 px-1">
                          {isViewMode ? (
                            ...
                            <select
                              ...
                              onChange={e => updateSalesItem(item.id, 'discountType', e.target.value)}
