...
export default function SalesForm({
  ...
  const [isSalesItemsCollapsed, setIsSalesItemsCollapsed] = useState(false)
              ...
              <div className="p-3.5 pt-1 flex-shrink-0">
                ...
                <div className="p-3 bg-gradient-to-br from-red-600 via-red-600 to-red-700 rounded-lg text-white shadow-md border-l-4 border-zinc-900 relative overflow-hidden">
                  ...
                  <div className="text-right w-full min-w-0">
                    <TotalHeroAmount value={form.summary.totalAmountDue} fmt={fmt} />
            ...
            >
                  ...
                  <div className="grid gap-4 grid-cols-5">
                    ...
                    <div>
                      ...
                      {isViewMode ? (
                        ...
                        <SearchableDropdown
                          ...
                          inputClassName={`w-full bg-white border rounded-lg px-3 py-2 text-sm text-zinc-800 focus:ring-2 focus:ring-red-600 focus:border-red-600 outline-none transition-all ${!form.customerSearch ? 'border-red-500' : 'border-zinc-300'}`}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled={isViewMode}
                        className={`w-full bg-white border rounded-lg px-3 py-2 text-sm text-zinc-800 focus:ring-2 focus:ring-red-600 focus:border-red-600 outline-none transition-all ${isViewMode ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''} ${!form.documentReference ? 'border-red-500' : 'border-zinc-300'}`}
                      />
                    </div>
                    {/* Terms */}
                    <div>
              ...
              <section className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                <div className="px-4 py-2.5 bg-zinc-900 text-white flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h2 className="text-base font-bold tracking-tight">Sales Items</h2>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                      ...
                      <div className="pl-7">
                        <SearchableDropdown
                          ...
                          onSelect={(opt) => {
                            ...
                            form.journalEntries.forEach((entry) =>
                              form.updateJournalEntry(entry.id, 'center', opt.value),
                            )
                          }}
                    ...
                    <button
                      onClick={() => setIsSalesItemsCollapsed(!isSalesItemsCollapsed)}
                      className="text-white bg-red-600 hover:bg-red-700 p-2 rounded-lg transition-colors"
                      title={isSalesItemsCollapsed ? 'Expand' : 'Collapse'}
                    >
                      {isSalesItemsCollapsed ? <ChevronDown size={16} /> : <ChevronUp size={16} />}
                    </button>
                  </div>
                </div>

                {!isSalesItemsCollapsed && (
                  <>
                ...
                {!isViewMode && (
                  <div className="p-4 bg-zinc-50 border-t border-zinc-200 flex items-center justify-between">
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          form.addSalesItem(false, bulkResponsibilityCenter)
                        }
                      ...
                      <button
                        onClick={() =>
                          form.addSalesItem(true, bulkResponsibilityCenter)
                        }
                    ...
                    <span className="text-xs text-zinc-500 font-medium">
                      {form.salesItems.length} {form.salesItems.length === 1 ? 'item' : 'items'} added
                    </span>
                  </div>
                )}
                  </>
              ...
              <section className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-zinc-800 text-zinc-100 border border-zinc-700`}>
                        {(() => {
                          const totalDebit = form.journalEntries.reduce(
                            (s, e) => s + (parseFloat(e.debit) || 0),
                            0,
                          )
                          const totalCredit = form.journalEntries.reduce(
                ...
                {!isViewMode && (
                  <div className="p-4 bg-zinc-50 border-t border-zinc-200 flex items-center justify-between">
                    <button
                      onClick={() => form.addJournalEntry(bulkResponsibilityCenter)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-red-500 border-dashed text-xs font-bold rounded-lg text-red-600 bg-red-50 hover:bg-red-100 transition-colors"
                    >
                      <Plus size={12} /> Add Ledger Row
                    </button>
                    <span className="text-xs text-zinc-500 font-medium">{form.journalEntries.length} {form.journalEntries.length === 1 ? 'entry' : 'entries'}</span>
                  </div>
                )}
                  </div>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <section className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden">
                  <div className="px-4 py-2.5 bg-zinc-900 text-white flex items-center justify-between">
                    ...
                    <span className="text-xs text-zinc-400 font-medium">{form.attachments.length} {form.attachments.length === 1 ? 'File' : 'Files'}</span>
                  </div>
                  <div className="p-4">
