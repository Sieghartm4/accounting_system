...
// ─────────────────────────────────────────────────────────────────────────────
export function useReceiptsForm({ isViewMode, isEditMode, receiptData, onBack, onSuccess }) {
  ...
  useEffect(() => {
    if ((isViewMode || isEditMode) && receiptData) {
      ...
      if (receiptData.journal && receiptData.journal.length > 0) {
        setJournalEntries(receiptData.journal.map(entry => ({
          id: entry.id,
      ...
      if (receiptData.attachments && receiptData.attachments.length > 0) {
        setAttachments(receiptData.attachments.map(att => ({
          id: att.id,
          fileName: att.name || '',
          file: att.file || null,
          remarks: att.remarks || '',
  ...
  const addReceiptItem = (isOther = false) =>
    setReceiptItems(prev => [...prev, { id: Date.now(), productId: '', productSearch: '', coa: '', coaSearch: '', description: '', qty: 1, price: 0, discount: 0, discountType: 'PERCENT', vat: 0, vatSearch: '', vatRate: 0, wht: 0, whtSearch: '', whtRate: 0, responsibilityCenter: '', isOther, isNew: true }]);
  ...
  const addJournalEntry = () =>
    setJournalEntries(prev => [...prev, { id: Date.now(), account: '', accountSearch: '', center: '', debit: 0, credit: 0, isManual: true, isNew: true }]);
  ...
  // ── Attachments CRUD ──
  const addAttachment = () =>
    setAttachments(prev => [...prev, { id: Date.now(), fileName: '', file: null, remarks: '', uploadedBy: 'Current User', date: new Date().toLocaleDateString(), isNew: true }]);
  ...
  const isAutoGeneratedEntry = (entry) => {
    ...
    if (entry.isManual === undefined) {
      ...
      const matchesVat = receiptItems.some(item => {
        ...
        return vatAmount > 0 && Math.abs(vatAmount - entryAmount) < 0.01 && entry.account && entry.account.toString().toLowerCase().includes('vat');
      });
      const matchesWht = receiptItems.some(item => {
        const whtAmount = parseFloat(item.wht) || 0;
        return whtAmount > 0 && Math.abs(whtAmount - entryAmount) < 0.01 && entry.account && entry.account.toString().toLowerCase().includes('wht');
      });
      ...
      const isBankEntry = entry.account && (entry.account.toString().toLowerCase().includes('cash') || entry.account.toString().toLowerCase().includes('bank')) && Math.abs(totalReceiptAmount - entryAmount) < 0.01;
  ...
  const generateJournalEntries = useCallback(() => {
    ...
    receiptItems.forEach((item) => {
      ...
      if (selectedCoa && gross > 0) {
        entries.push({ id: Date.now() + Math.random(), account: selectedCoa.id, accountSearch: selectedCoa.name, center: item.responsibilityCenter || '', debit: 0, credit: parseFloat(gross.toFixed(2)), isManual: false });
      }

      if (vatAmount > 0) {
        const outputVatAccount = chartsOfAccounts.find(a => (a.name || '').toLowerCase().includes('output vat'));
        if (outputVatAccount) {
          entries.push({ id: Date.now() + Math.random(), account: outputVatAccount.id, accountSearch: outputVatAccount.name, center: item.responsibilityCenter || '', debit: 0, credit: parseFloat(vatAmount.toFixed(2)), isManual: false });
        }
      }
      ...
      if (whtAmount > 0) {
        ...
        if (whtAccount) {
          entries.push({ id: Date.now() + Math.random(), account: whtAccount.id, accountSearch: whtAccount.name, center: item.responsibilityCenter || '', debit: parseFloat(whtAmount.toFixed(2)), credit: 0, isManual: false });
        }
      }

      if (discountAmount > 0) {
        const discountAccount = chartsOfAccounts.find(a => (a.name || '').toLowerCase().includes('sales discounts'));
        if (discountAccount) {
          entries.push({ id: Date.now() + Math.random(), account: discountAccount.id, accountSearch: discountAccount.name, center: item.responsibilityCenter || '', debit: parseFloat(discountAmount.toFixed(2)), credit: 0, isManual: false });
        }
      }
    });
    ...
    if (paymentAccount && totalCashAmount > 0) {
      entries.push({ id: Date.now() + Math.random(), account: paymentAccount.id, accountSearch: paymentAccount.name, center: '', debit: parseFloat(totalCashAmount.toFixed(2)), credit: 0, isManual: false });
    }
  ...
  const handlePostTransaction = async () => {
    ...
    try {
      ...
      const hasValidItems = receiptItems.some(item =>
        item.isOther ? item.description.trim() !== '' && item.coa !== '' : item.productId !== ''
      );
      if (!hasValidItems) { setToast({ type: 'warning', message: 'Please add at least one valid receipt item' }); return; }
      ...
      const preparedAttachments = await Promise.all(
        attachments.map(async att => {
          ...
          return { id: att.id && !att.isNew ? att.id : null, fileName: att.fileName, file: fileData, remarks: att.remarks, uploadedBy: att.uploadedBy, date: att.date };
        })
      );
      ...
      if (isEditMode && receiptId) { requestData.updated_by = createdBy; } else { requestData.created_by = createdBy; }
      ...
      if (result.success) {
        const nextToast = { type: 'success', message: isEditMode ? 'Receipt updated successfully!' : 'Receipt created successfully!' };
        setToast(nextToast);
        if (onSuccess) await onSuccess(nextToast);
        onBack();
      } else {
        setToast({ type: 'error', message: result.message || (isEditMode ? 'Failed to update receipt' : 'Failed to create receipt') });
      }
    } catch (error) {
...
const useReceipts = () => {
  ...
  const refetchReceipts = useCallback(async () => {
    try {
      ...
      const result = await response.json();
      if (result.success) { setReceipts(result.data); } else { setError(result.message || 'Failed to fetch receipts'); }
    } catch (err) {
  ...
  useEffect(() => { refetchReceipts(); }, [refetchReceipts]);
