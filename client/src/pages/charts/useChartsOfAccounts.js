...
const useChartsOfAccounts = () => {
  ...
  const updateChartsOfAccount = async (id, code, name, type, description, status) => {
    try {
      const token = localStorage.getItem("token");
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_LINK}/charts_of_accounts/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (result.success) {
        ...
        return { success: false, message: result.message || "Failed to update charts of account" };
      }
    } catch (err) {
      console.error('Error updating charts of account:', err.message);
      return { success: false, message: err.message };
    }
  };

  return { chartsOfAccounts, loading, error, createChartsOfAccount, updateChartsOfAccount };
};
