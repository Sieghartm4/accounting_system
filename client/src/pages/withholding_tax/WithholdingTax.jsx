...
function WithholdingTaxContent() {
  ...
  const handleImportWithholdingTax = async (e) => {
    ...
    try {
      ...
      const taxes = parseCSV(text)

      if (taxes.length === 0) {
        setToast({ type: 'error', message: 'No valid tax entries found in CSV' })
        return
      }

      const result = await importWithholdingTax(taxes)
      ...
      if (result.success) {
        ...
        setToast({
          type: 'error',
          message: result.message || 'Failed to import withholding tax entries',
        })
      }
    } catch (err) {
  ...
  if (loading) {
    return <LoadingScreen label="Loading Withholding Tax Data..." />
  }
            ...
            <ProtectedAction routeName="witholding_tax">
              <label className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition-all shadow-sm cursor-pointer">
                ...
                <input
                  type="file"
                  accept=".csv"
                  onChange={handleImportWithholdingTax}
                  className="hidden"
                  disabled={importing}
                />
              </label>
