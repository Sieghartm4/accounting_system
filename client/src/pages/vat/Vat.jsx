...
import { FileText, Percent, ShieldCheck, Clock, ArrowRight, Download, Plus, X, Edit2, Save } from 'lucide-react';
...
function VatContent() {
  const { vat, loading, error, createVatEntry, updateVatEntry, refreshVat } = useVat();
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      setFormData({ id: '', code: '', name: '', rate: '', type: '', sub_type: '', description: '', status: 'ACTIVE' });
      setEditingVat(null);
      setIsModalOpen(false);
    } else {
      setToast({ type: 'error', message: result.error || `Failed to ${editingVat ? 'update' : 'create'} VAT entry` });
    }
  ...
  if (loading) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs font-black uppercase tracking-[3px] text-gray-400">Fetching VAT Data...</p>
      </div>
    );
  }
