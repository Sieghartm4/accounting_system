...
export default function CashDisbursementForm({ onBack, onSuccess, isViewMode = false, isEditMode = false, disbursementData = null }) {
  ...
  const handlePostTransaction = async () => {
    try {
      ...
      const hasValidItems = disbursementItems.some(item => {
        if (item.isOther) {
              ...
              <TableSection title="Disbursement Items" icon={<Wallet size={14} />}>
                      ...
                      {disbursementItems.map((item) => (
                        <tr key={item.id} className={item.isOther ? 'bg-gray-50/30' : ''}>
                          <td className="py-1 px-1">
                            {item.isOther ? (
                              <div className="cursor-not-allowed text-center text-gray-400 text-[11px] italic py-2"></div>
                            ) : (
                              <SearchableDropdown
                                disabled={isViewMode}
                ...
                {!isViewMode && (
                  <div className="flex gap-2 mt-3">
                    <button
                      onClick={() => addDisbursementItem(false)}
                    ...
                    <button
                      onClick={() => addDisbursementItem(true)}
