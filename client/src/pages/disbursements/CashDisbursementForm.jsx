...
import { useDisbursementForm, useDragToScroll, fmt } from './useDisbursements';
...
export default function CashDisbursementForm({ onBack, onSuccess, isViewMode = false, isEditMode = false, disbursementData = null }) {
  const {
    ...
    summary,
  } = useDisbursementForm({ isViewMode, isEditMode, disbursementData, onBack, onSuccess });
        ...
        <fieldset className="bg-black rounded-2xl p-3 pl-6 pr-6 text-white shadow-xl">
          ...
          <div className={`grid gap-3 ${(modeOfPayment === 'CHECK' || modeOfPayment === 'BANK_TRANSFER') ? 'grid-cols-6' : 'grid-cols-4'}`}>
            <fieldset className="col-span-1">
              <legend className="text-[11px] font-black uppercase text-gray-100">Vendor / Payee <span className="text-red-600">*</span></legend>
              {isViewMode ? (
                ...
                <SearchableDropdown placeholder="Vendor / Payee" value={vendorSearch} onChange={v => { setVendorSearch(v); setSelectedVendor(''); }} onSelect={opt => { setSelectedVendor(opt.value); setVendorSearch(opt.label); }} options={vendorOptions} inputClassName={inputBase} emptyText={vendorError || 'No vendors found'} />
              )}
            </fieldset>

            <fieldset className="col-span-1">
              <legend className="text-[11px] font-black uppercase text-gray-100">Reference</legend>
              <input type="text" placeholder="Reference" value={documentReference} onChange={e => setDocumentReference(e.target.value)} disabled={isViewMode} className={`w-full px-3 py-1.5 rounded-lg text-[12px] font-bold outline-none transition-all ${isViewMode ? 'bg-gray-100 border border-gray-300 text-black cursor-not-allowed' : 'bg-white border border-gray-200 text-black focus:ring-1 focus:ring-red-500'}`} />
            </fieldset>
            ...
            <fieldset>
              ...
              {isViewMode ? (
                ...
                <SearchableDropdown placeholder="Mode of Payment" value={modeSearch} onChange={v => { setModeSearch(v); setModeOfPayment(''); }} onSelect={opt => { setModeOfPayment(opt.value); setModeSearch(opt.label); }} options={modeOfPaymentOptions.map(m => ({ label: m, value: m }))} inputClassName={inputBase} emptyText="No modes found" />
              )}
            </fieldset>

            <fieldset>
              <legend className="text-[11px] font-black uppercase text-gray-100">Date</legend>
              <input type="date" value={paymentDate} onChange={e => setPaymentDate(e.target.value)} disabled={isViewMode} className={`w-full px-3 py-1.5 rounded-lg text-[12px] font-bold outline-none transition-all ${isViewMode ? 'bg-gray-100 border border-gray-300 text-black cursor-not-allowed' : 'bg-white border border-gray-200 text-black focus:ring-1 focus:ring-red-500'}`} />
            </fieldset>
            ...
            {(modeOfPayment === 'CHECK' || modeOfPayment === 'BANK_TRANSFER') && (
              <fieldset>
                ...
                <input type="text" placeholder="Bank Name" value={bankName} onChange={e => setBankName(e.target.value)} disabled={isViewMode} className={`w-full px-3 py-1.5 rounded-lg text-[12px] font-bold outline-none transition-all ${isViewMode ? 'bg-gray-100 border border-gray-300 text-black cursor-not-allowed' : 'bg-white border border-gray-200 text-black focus:ring-1 focus:ring-red-500'}`} />
              </fieldset>
            )}
            ...
            {(modeOfPayment === 'CHECK' || modeOfPayment === 'BANK_TRANSFER') && (
              <fieldset>
                ...
                <input type="text" placeholder="Check #" value={checkNumber} onChange={e => setCheckNumber(e.target.value)} disabled={isViewMode} className={`w-full px-3 py-1.5 rounded-lg text-[12px] font-bold outline-none transition-all ${isViewMode ? 'bg-gray-100 border border-gray-300 text-black cursor-not-allowed' : 'bg-white border border-gray-200 text-black focus:ring-1 focus:ring-red-500'}`} />
              </fieldset>
            )}
          </div>
        </fieldset>
            ...
            <section className="bg-white rounded-2xl border-2 border-red-100 shadow-xl shadow-red-500/5 flex-1 flex flex-col min-h-0 overflow-hidden">
              ...
              <div className="custom-table-scroller overflow-y-auto min-h-0 flex-1 custom-scrollbar p-4 py-2">
                <div className="space-y-0">
                  <SummaryRow label="Total Purchase Price" value={fmt(summary.totalPurchasePrice)} />
                  <SDivider />
                  ...
                  <SDivider />
                  <SummaryRow label="VATable Purchases" value={fmt(summary.vatablePurchases)} />
                  <SDivider />
                  <SummaryRow label="VAT-Exempt Purchases" value={fmt(summary.vatExemptPurchases)} />
                  <SDivider />
                  <SummaryRow label="Zero Rated Purchases" value={fmt(summary.zeroRatedPurchases)} />
                  <SDivider />
              ...
              <div className="p-4 pt-0 flex-shrink-0">
                ...
                <div className="text-center bg-red-500 rounded-xl py-3 border border-gray-100">
                  <p className="text-[clamp(11px,1.1vw,12px)] font-black text-gray-100 uppercase tracking-[4px] mb-1">Total Cash Disbursement</p>
            ...
            <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="space-y-4">
                        ...
                        <tr key={item.id} className={item.isOther ? 'bg-gray-50/30' : ''}>
                          <td className="py-1 px-1">
                            {item.isOther ? <div className="cursor-not-allowed text-center text-gray-400 text-[11px] italic py-2" /> : (
                              <SearchableDropdown disabled={isViewMode} placeholder="Search product..." value={item.productSearch} onChange={v => updateDisbursementItem(item.id, 'productSearch', v)} onSelect={opt => { updateDisbursementItem(item.id, 'productId', opt.value); updateDisbursementItem(item.id, 'productSearch', opt.label); }} options={productOptions} inputClassName={`${tableInput} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} emptyText={productError || 'No products found'} />
                            )}
                          </td>
                          <td className="py-1 px-1">
                          ...
                          <td className="py-1 px-1">
                            <div className="relative">
                              <input disabled={isViewMode} className={`${tableInput + ' font-black'} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} type="number" min="0" max={item.discountType === 'PERCENT' ? '100' : '999999'} step="0.01" placeholder="0" value={item.discount === null || item.discount === undefined ? '' : item.discount} onChange={e => updateDisbursementItem(item.id, 'discount', e.target.value === '' ? null : parseFloat(e.target.value) || 0)} />
                              {item.discountType === 'PERCENT' && <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 font-black pointer-events-none">%</span>}
                            </div>
                          </td>
                          <td className="py-1 px-1">
                            {isViewMode ? (
                              <div className={`${tableInput} text-black py-1.5 text-center`}>{item.discountType === 'PERCENT' ? 'Percentage' : 'Fixed'}</div>
                            ) : (
                              <select value={item.discountType || 'PERCENT'} onChange={e => updateDisbursementItem(item.id, 'discountType', e.target.value)} className="w-full px-2 py-1 text-[11px] font-bold border border-gray-200 rounded focus:ring-1 focus:ring-red-400 outline-none">
                          ...
                          <td className="py-1 px-1">
                            {isViewMode ? (
                              ...
                              <SearchableDropdown placeholder="VAT Rate" value={item.vatSearch} onChange={v => updateDisbursementItem(item.id, 'vatSearch', v)} onFocus={loadVatOnDemand} onSelect={opt => { updateDisbursementItem(item.id, 'vat', opt.value); updateDisbursementItem(item.id, 'vatSearch', opt.label); updateDisbursementItem(item.id, 'vatRate', opt.rate); }} options={vatOptions} inputClassName={`${pctInput + ' font-black text-red-600'}`} emptyText={vatError || 'No VAT rates found'} disabled={vatLoading} />
                            )}
                          </td>
                          <td className="py-1 px-1">
                            {isViewMode ? (
                              ...
                              <SearchableDropdown placeholder="WHT Rate" value={item.whtSearch} onChange={v => updateDisbursementItem(item.id, 'whtSearch', v)} onFocus={loadWhtOnDemand} onSelect={opt => { updateDisbursementItem(item.id, 'wht', opt.value); updateDisbursementItem(item.id, 'whtSearch', opt.label); updateDisbursementItem(item.id, 'whtRate', opt.rate); }} options={whtOptions} inputClassName={`${pctInput + ' font-black text-blue-600'}`} emptyText={whtError || 'No WHT rates found'} disabled={whtLoading} />
                            )}
                          </td>
                          <td className="py-1 px-1">
                          ...
                          <td className="py-1 px-1 text-center">
                            {!isViewMode && <button onClick={() => removeDisbursementItem(item.id)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors"><Trash2 size={15} /></button>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <TableSection title="Journal Entries" icon={<Layers size={14} />}>
                ...
                <div className="overflow-x-auto custom-table-scroller">
                  <table className="w-full text-center" style={{ tableLayout: 'fixed', minWidth: 600 }}>
                    <colgroup>
                      <col style={{ width: '40%' }} /><col style={{ width: '16%' }} /><col style={{ width: '16%' }} />
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {journalEntries.map((entry) => (
                        <tr key={entry.id}>
                          ...
                          <td className="py-1.5 px-1">
                            <input disabled={isViewMode} className={`${tableInput + ' font-black'} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} placeholder="0.00" type="number" value={entry.debit || ''} onChange={e => updateJournalEntry(entry.id, 'debit', e.target.value === '' ? '' : parseFloat(e.target.value) || 0)} />
                          </td>
                          <td className="py-1.5 px-1">
                            <input disabled={isViewMode} className={`${tableInput + ' font-black text-red-600'} ${isViewMode ? 'bg-transparent cursor-not-allowed' : ''}`} placeholder="0.00" type="number" value={entry.credit || ''} onChange={e => updateJournalEntry(entry.id, 'credit', e.target.value === '' ? '' : parseFloat(e.target.value) || 0)} />
                          </td>
                          <td className="py-1.5 px-1">
                            <input disabled={isViewMode} className={`${tableInput} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} placeholder="Center..." value={entry.center} onChange={e => updateJournalEntry(entry.id, 'center', e.target.value)} />
                          </td>
                          <td className="py-1.5 text-center">
                            {!isViewMode && <button className="p-1 text-red-600 transition-colors hover:bg-red-50 rounded" onClick={() => removeJournalEntry(entry.id)}><Trash2 size={15} className="mx-auto" /></button>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {!isViewMode && (
                  <div className="flex gap-2 mt-3">
                    <button onClick={addJournalEntry} className="flex-1 py-2 border-2 border-dashed rounded-xl text-[11px] font-black uppercase border-red-300 text-red-600 transition-all duration-300 hover:bg-red-50 hover:border-red-500 hover:-translate-y-1 hover:shadow-lg hover:shadow-red-500/10 flex items-center justify-center gap-2"><Plus size={14} /> ADD Ledger Row</button>
                  </div>
                )}
              </TableSection>
              ...
              <div className="grid grid-cols-1 gap-4">
                <TableSection title="Attachments" icon={<Paperclip size={14} />}>
                        ...
                        {attachments.map((file) => (
                          <tr key={file.id}>
                            <td className="py-2 px-1">
                              <input disabled={isViewMode} className={`${tableInput} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} placeholder="e.g. Invoice_Scan" value={file.fileName} onChange={(e) => updateAttachment(file.id, 'fileName', e.target.value)} />
                            </td>
                            <td className="py-2 px-1">
                              {isViewMode ? (
                            ...
                            <td className="py-2 px-1">
                              <input disabled={isViewMode} className={`${tableInput} ${isViewMode ? 'bg-transparent text-black cursor-not-allowed' : ''}`} placeholder="Add note..." value={file.remarks} onChange={(e) => updateAttachment(file.id, 'remarks', e.target.value)} />
                            </td>
                            ...
                            <td className="py-2 text-center">
                              {!isViewMode && <button onClick={() => removeAttachment(file.id)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors"><Trash2 size={15} /></button>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  {!isViewMode && (
                    <div className="flex gap-2 mt-3">
                      <button onClick={addAttachment} className="flex-1 py-2 border-2 border-dashed rounded-xl text-[11px] font-black uppercase border-red-300 text-red-600 transition-all duration-300 hover:bg-red-50 hover:border-red-500 hover:-translate-y-1 hover:shadow-lg hover:shadow-red-500/10 flex items-center justify-center gap-2"><Plus size={14} /> ADD File</button>
                    </div>
                  )}
                </TableSection>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between p-4">
        ...
        <button type="button" onClick={() => setIsCollapsed(v => !v)} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg transition-colors">
