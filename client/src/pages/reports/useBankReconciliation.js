...
export function useBankReconciliation(selectedReconciliation) {
  ...
  const [adjustmentFormRows, setAdjustmentFormRows] = useState([
    emptyAdjustmentForm(),
  ])
  ...
  const fetchAdjustments = async (start = detailStartDate, end = detailEndDate) => {
    try {
      setAdjustmentsLoading(true)
      const token = localStorage.getItem('token')
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_LINK}/bank_reconciliation/${selectedReconciliation.id}/adjustments${buildDateQuery(start, end)}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
      ...
      if (result.success) {
        ...
        showToastMsg('Failed to load adjustments', 'error')
      }
    } catch {
      showToastMsg('Failed to load adjustments', 'error')
    } finally {
  ...
  const handleAddAdjustment = async () => {
    const hasInvalidRow = adjustmentFormRows.some((row) => {
      const amount = parseFloat(row.amount) || 0
      return !row.date || !row.type || amount <= 0
    })

    if (hasInvalidRow) {
      showToastMsg(
        'Each row must have a Date, Type, and Amount greater than 0',
        'error',
      )
      return
    }
    ...
    try {
      ...
      for (const row of adjustmentFormRows) {
        const bodyData = {
          date: row.date,
          coa_id: reconData.coa_id,
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SERVER_LINK}/bank_reconciliation/${selectedReconciliation.id}/adjustment/add`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        const result = await response.json()
        if (!result.success) {
          showToastMsg(result.message || 'Failed to save adjustment', 'error')
          return
        }
      }

      showToastMsg(`${adjustmentFormRows.length} adjustment(s) saved successfully`)
      await fetchAdjustments()
      if (reconData.coa_id) {
        await fetchJournalEntriesByCoa(
          reconData.coa_id,
          detailStartDate,
          detailEndDate,
        )
      ...
      showToastMsg('Server error while saving adjustment', 'error')
    }
  }
  ...
  const resetAdjustmentForm = () => {
    setAdjustmentFormRows([emptyAdjustmentForm()])
  }
  ...
  const updateAdjustmentFormRow = (index, field, value) => {
    setAdjustmentFormRows((prevRows) =>
  ...
  const addAdjustmentFormRow = () => {
    setAdjustmentFormRows((prevRows) => [...prevRows, emptyAdjustmentForm()])
  }
  ...
  const removeAdjustmentFormRow = (index) => {
    setAdjustmentFormRows((prevRows) =>
      prevRows.length === 1 ? prevRows : prevRows.filter((_, i) => i !== index),
    )
  }
  ...
  const adjustmentDebits = adjustments
    .filter((e) => e.type?.toLowerCase() === 'debit')
    .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)

  const adjustmentCredits = adjustments
    .filter((e) => e.type?.toLowerCase() === 'credit')
    .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)

  const adjustedBankBalance =
    bankStatementEndingBalance + bankAdditions - bankDeductions
  ...
  const adjustedBookBalance =
    endingBookBalance + bookAdditions - bookDeductions + bookErrorAdjustments
  ...
  const adjSearch = adjustmentSearchTerm.toLowerCase().trim()
  const visibleAdjustments = adjSearch
    ? adjustments.filter((entry) =>
        [
          entry.date,
          ...
          .filter(Boolean)
          .some((v) => String(v).toLowerCase().includes(adjSearch)),
      )
    : adjustments
  ...
  return {
    ...
    adjustmentDebits,
    adjustmentCredits,
    adjDebits: adjustmentDebits,
    adjCredits: adjustmentCredits,
    BANK_SECTION_ITEMS,
    BOOK_SECTION_ITEMS,
