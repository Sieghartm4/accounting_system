...
const { corsOptions } = require('../middlewares/corsOptions.middleware')
...
const initRoutes = (app) => {
  // ✅ Use proper CORS middleware (handles credentials + specific origins)
  ...
  app.get('/admin', (req, res) => {
    let clientUrl =
      process.env._CLIENT_URL || `://${process.env._SERVER_URL || 'localhost'}:5050`
    ...
    const html = require('fs').readFileSync(
      './src/views/subscription-admin.html',
      'utf8',
    )
    const renderedHtml = html.replace(
      '<head>',
      `<head><script>window.CLIENT_URL = '${clientUrl}'; console.log('CLIENT_URL from server:', window.CLIENT_URL);</script>`,
    )
